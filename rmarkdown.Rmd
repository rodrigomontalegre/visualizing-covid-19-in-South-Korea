...
plot_cases_popdensity <- plot_a + geom_line(cases_pop_density, 
  ...
  labs(title = "Number of confirmed COVID-19 cases versus population density",
       x = "South Korean province",
       y = "Number of cases") +
...
qqplot1 <- ggplot(data = cases_pop_density, aes(sample = accumulated_sum)) + geom_qq() + stat_qq_line() +
  labs(title = "Q-Q PLot (Cases data)")
...
The final hypothesis is that different age groups are affected by different infection durations. We used the patient information dataset to calculate an average duration per age group, which in our case is defined as the release date - confirmed date. After cleaning the dataset and disregarding patients of unknown ages, we can plot the following graph.
```{r, echo = FALSE, message = FALSE, include = FALSE}
...
summary(lm1)
```
The linear regression model suggests that while the effect of age on the duration of infection is statistically signifacant, its effect is very small and it does not explain the variance in the model.